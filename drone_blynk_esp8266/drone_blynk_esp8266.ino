...
//D2 (GPIO4)  SDA
//-------------------
//D5 (GPIO14)  ESC1   Motor1   GND
//D6 (GPIO12)  ESC2   Motor2   GND
//D7 (GPIO13)  ESC3   Motor3   GND
//D8 (GPIO15)  ESC4   Motor4   GND
// Blynk authentication token (get this from the Blynk app)
...
#define THROTTLE_JOYSTICK V0  // Throttle control (Y-axis)
#define YAW_JOYSTICK      V1   // Yaw control (X-axis)
#define ROLL_JOYSTICK     V2   // Roll control (X-axis)
#define PITCH_JOYSTICK    V3   // Pitch control (Y-axis)
#define MODE_SWITCH       V4   // Mode switch
...
void ICACHE_RAM_ATTR PWM_callback() {
  switch (pwm_stops) {
    case 0:
      ...
      pwm_stops = 1;
      if (input_THROTTLE != 0) {
        GPOS = (1 << 14); GPOS = (1 << 12); GPOS = (1 << 15); GPOS = (1 << 13);
      }
...
BLYNK_WRITE(THROTTLE_JOYSTICK) {
  // Y-axis of first joystick (throttle)
  input_THROTTLE = map(param[1].asInt(), 0, 255, 0, 2400); // Map from 0-255 to 0-2400
}

BLYNK_WRITE(YAW_JOYSTICK) {
  // X-axis of first joystick (yaw)
  input_YAW = map(param[0].asInt(), 0, 255, 0, 100); // Map from 0-255 to 0-100
}

BLYNK_WRITE(ROLL_JOYSTICK) {
  // X-axis of second joystick (roll)
  input_ROLL = map(param[0].asInt(), 0, 255, 0, 100); // Map from 0-255 to 0-100
}

BLYNK_WRITE(PITCH_JOYSTICK) {
  // Y-axis of second joystick (pitch)
  input_PITCH = map(param[1].asInt(), 0, 255, 0, 100); // Map from 0-255 to 0-100
}
...
void loop() {
  ...
  if (Mode == 1 && (abs(input_ROLL - 50) > 30 || abs(input_PITCH - 50) > 30)) {
    wall_car_init = true;
    if (input_ROLL > 30) {
      target_axis = 1; target_dirr = 1;
    }
    else if (input_ROLL < -30) {
      target_axis = 1; target_dirr = -1;
    }
    else if (input_PITCH > 30) {
      target_axis = 2; target_dirr = 1;
    }
    else if (input_PITCH < -30) {
      target_axis = 2; target_dirr = -1;
    }
    ...
    wall_car_init = false; set_motor_const_speed = false;
  }
